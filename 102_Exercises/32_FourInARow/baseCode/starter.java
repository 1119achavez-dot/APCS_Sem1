...
class starter {
	...
	// Any spot with an "O" in its place should be disregarded and continued to the next check
 	public static int checkVertical(String[][] board){
		...
		for(int r = 0; r < board.length -3; r++){ // row go thrrough almost all 
			for(int c= 0; c < board[0].length; c++){ // coloum 
		
				
				if{ (board[r][c].equals(board[r+1][c] && 
					board[r+1][b].equals(board[r+2][c] && 
...
// wring my method here , paste somewhere later
